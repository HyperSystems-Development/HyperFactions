...
public class JsonEconomyStorage {
    ...
    public CompletableFuture<Void> save(@NotNull UUID factionId, @NotNull FactionEconomy economy) {
        return CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                Logger.severe("Failed to save economy for faction %s: %s", factionId, e.getMessage());
            }
        });
    }
    ...
    public CompletableFuture<Void> delete(@NotNull UUID factionId) {
        return CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                Logger.severe("Failed to delete economy for faction %s: %s",
    ...
    private JsonObject serialize(@NotNull FactionEconomy economy) {
        ...
        obj.addProperty("balance", economy.balance());

        // Limits
        TreasuryLimits limits = economy.limits();
        JsonObject limitsObj = new JsonObject();
        limitsObj.addProperty("maxWithdrawAmount", limits.maxWithdrawAmount());
        limitsObj.addProperty("maxWithdrawPerPeriod", limits.maxWithdrawPerPeriod());
        limitsObj.addProperty("maxTransferAmount", limits.maxTransferAmount());
        limitsObj.addProperty("maxTransferPerPeriod", limits.maxTransferPerPeriod());
        ...
        for (EconomyAPI.Transaction tx : economy.transactionHistory()) {
            ...
            txObj.addProperty("balanceAfter", tx.balanceAfter());
    ...
    private FactionEconomy deserialize(@NotNull JsonObject obj) {
        ...
        if (obj.has("limits")) {
            ...
            limits = new TreasuryLimits(
                    getDouble(limitsObj, "maxWithdrawAmount", 0),
                    getDouble(limitsObj, "maxWithdrawPerPeriod", 0),
                    getDouble(limitsObj, "maxTransferAmount", 0),
                    getDouble(limitsObj, "maxTransferPerPeriod", 0),
        ...
        if (obj.has("transactions")) {
            ...
            for (JsonElement elem : txArray) {
                ...
                double amount = txObj.get("amount").getAsDouble();
                double balanceAfter = txObj.get("balanceAfter").getAsDouble();
