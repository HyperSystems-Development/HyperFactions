...
public class TreasuryTransferConfirmPage extends InteractiveCustomUIPage<TreasuryTransferConfirmData> {
  ...
  public void build(Ref<EntityStore> ref, UICommandBuilder cmd,
    ...
    String typeTag = "player".equals(targetType)
        ? HFMessages.get(playerRef, MessageKeys.TreasuryGui.TAG_PLAYER)
        : HFMessages.get(playerRef, MessageKeys.TreasuryGui.TAG_FACTION);
    ...
    cmd.set("#TreasuryLabel.Text", HFMessages.get(playerRef, MessageKeys.TreasuryGui.TREASURY_LABEL, economyManager.formatCurrency(treasuryBalance)));
  ...
  private void handleConfirm(Player player, Ref<EntityStore> ref, Store<EntityStore> store,
    ...
    if (!PermissionManager.get().hasPermission(uuid, Permissions.ECONOMY_TRANSFER)) {
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.NO_TRANSFER_PERMISSION));
      sendUpdate();
      return;
    }
    ...
    if (limitReason != null) {
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.TRANSFER_DENIED, limitReason));
      sendUpdate();
      return;
    }
  ...
  private void handleFactionTransfer(Player player, Ref<EntityStore> ref, Store<EntityStore> store,
    ...
    try {
      ...
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.INVALID_TARGET_FACTION));
      sendUpdate();
      return;
    }
    ...
    if (targetFaction == null) {
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.TARGET_FACTION_GONE));
      sendUpdate();
      return;
    }
    ...
    if (withdrawResult != EconomyAPI.TransactionResult.SUCCESS) {
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.TRANSFER_FAILED, withdrawResult));
      sendUpdate();
      return;
    }
    ...
    if (depositResult != EconomyAPI.TransactionResult.SUCCESS) {
      ...
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.TRANSFER_FAILED_RETURNED));
      sendUpdate();
      return;
    }

    player.sendMessage(MessageUtil.successText(playerRef, MessageKeys.TreasuryGui.TRANSFERRED,
  ...
  private void handlePlayerTransfer(Player player, Ref<EntityStore> ref, Store<EntityStore> store,
    ...
    try {
      ...
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.INVALID_TARGET_PLAYER));
      sendUpdate();
      return;
    }
    ...
    if (!vault.deposit(targetUuid, recipientReceives)) {
      ...
      player.sendMessage(MessageUtil.errorText(playerRef, MessageKeys.TreasuryGui.PLAYER_TRANSFER_FAILED));
      sendUpdate();
      return;
    }
