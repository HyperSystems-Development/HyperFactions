...
public class FactionRelationsPage extends InteractiveCustomUIPage<FactionRelationsData> implements RefreshablePage {
  ...
  /** Creates a new FactionRelationsPage. */
  ...
  private void buildList(UICommandBuilder cmd, UIEventBuilder events, boolean canManage) {
    ...
    cmd.set("#TabRelations.Style", Value.ref(UIPaths.STYLES,
        currentTab == Tab.RELATIONS ? "CyanButtonStyle" : "ButtonStyle"));
    cmd.set("#TabPending.Style", Value.ref(UIPaths.STYLES,
  ...
  private void buildEntry(UICommandBuilder cmd, UIEventBuilder events, int index,
    ...
    cmd.append("#IndexCards", UIPaths.FACTION_RELATION_ENTRY);
    ...
    String powerColor = GuiColors.forPowerRatio(powerRatio);
  ...
  private void handleViewFaction(Player player, Ref<EntityStore> ref, Store<EntityStore> store,
    ...
    if (targetId == null) {
      playerRef.sendMessage(MessageUtil.error(playerRef, GuiKeys.BrowserGui.INVALID_FACTION));
      sendUpdate();
      return;
    }
    ...
    if (targetFaction != null) {
      ...
      playerRef.sendMessage(MessageUtil.error(playerRef, GuiKeys.PlayerInfoGui.FACTION_GONE));
      sendUpdate();
    }
  }
  ...
  private void handleSetNeutral(Player player, FactionRelationsData data, boolean canManage) {
    ...
    if (result == RelationManager.RelationResult.SUCCESS) {
      playerRef.sendMessage(MessageUtil.info(playerRef, GuiKeys.RelationsGui.NOW_NEUTRAL, "#888888", data.factionName));
    } else {
      playerRef.sendMessage(MessageUtil.error(playerRef, GuiKeys.RelationsGui.FAILED, result));
    }
    rebuildList(canManage);
  }
  ...
  private void handleSetEnemy(Player player, FactionRelationsData data, boolean canManage) {
    ...
    if (result == RelationManager.RelationResult.SUCCESS) {
      playerRef.sendMessage(MessageUtil.error(playerRef, GuiKeys.RelationsGui.NOW_ENEMIES, data.factionName));
    } else {
      playerRef.sendMessage(MessageUtil.error(playerRef, GuiKeys.RelationsGui.FAILED, result));
    }
    rebuildList(canManage);
  ...
  private void handleRequestAlly(Player player, FactionRelationsData data, boolean canManage) {
    ...
    if (result == RelationManager.RelationResult.REQUEST_SENT) {
      playerRef.sendMessage(MessageUtil.info(playerRef, GuiKeys.RelationsGui.REQUEST_SENT, "#00AAFF", data.factionName));
      ...
      playerRef.sendMessage(MessageUtil.info(playerRef, GuiKeys.RelationsGui.NOW_ALLIED, "#00AAFF", data.factionName));
    } else {
      playerRef.sendMessage(MessageUtil.error(playerRef, GuiKeys.RelationsGui.FAILED, result));
    }
    rebuildList(canManage);
  ...
  private void handleDeclineAlly(Player player, FactionRelationsData data, boolean canManage) {
    ...
    playerRef.sendMessage(MessageUtil.info(playerRef, GuiKeys.RelationsGui.REQUEST_DECLINED, "#888888", data.factionName));
    rebuildList(canManage);
  }
  ...
  private void handleCancelRequest(Player player, FactionRelationsData data, boolean canManage) {
    ...
    if (result == RelationManager.RelationResult.SUCCESS) {
      playerRef.sendMessage(MessageUtil.info(playerRef, GuiKeys.RelationsGui.REQUEST_CANCELLED, "#888888", data.factionName));
    } else {
      playerRef.sendMessage(MessageUtil.error(playerRef, GuiKeys.RelationsGui.FAILED, result));
    }
