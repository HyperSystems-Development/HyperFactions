...
public class TreasuryDepositModalPage extends InteractiveCustomUIPage<DepositModalData> {
    ...
    public void build(Ref<EntityStore> ref, UICommandBuilder cmd,
        ...
        cmd.append("HyperFactions/faction/treasury_deposit_modal.ui");
        ...
        cmd.set("#WalletLabel.Text", "Your wallet: " + economyManager.formatCurrency(vault.getBalance(uuid)));
        ...
        double feePercent = isDeposit ? ConfigManager.get().getDepositFeePercent() : ConfigManager.get().getWithdrawFeePercent();
    ...
    private void handlePreview(DepositModalData data) {
        ...
        if (amount <= 0) {
            ...
            double fee = economyManager.calculateFee(amount, txType);
            double total;
            if (isDeposit) {
    ...
    private void handleDepositConfirm(Player player, Ref<EntityStore> ref, Store<EntityStore> store,
                                       PlayerRef playerRef, UUID uuid, double amount, double fee) {
        double totalFromWallet = amount + fee;
        VaultEconomyProvider vault = economyManager.getVaultProvider();
        ...
        if (!vault.has(uuid, totalFromWallet)) {
            player.sendMessage(MessageUtil.errorText(
                    ...
                    + ", have " + economyManager.formatCurrency(vault.getBalance(uuid))));
            sendUpdate();
            return;
        }
