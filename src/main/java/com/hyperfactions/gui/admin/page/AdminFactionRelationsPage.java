...
public class AdminFactionRelationsPage extends InteractiveCustomUIPage<AdminFactionRelationsData> {
    ...
    public void build(Ref<EntityStore> ref, UICommandBuilder cmd, UIEventBuilder events, Store<EntityStore> store) {
        ...
        if (faction == null) { cmd.set("#FactionName.Text", "Faction Not Found"); return; }
        ...
        else { for (int i = 0; i < allies.size(); i++) buildRelationEntry(cmd, events, "#AlliesList", i, allies.get(i), "ally"); }
        ...
        else { for (int i = 0; i < enemies.size(); i++) buildRelationEntry(cmd, events, "#EnemiesList", i, enemies.get(i), "enemy"); }
    ...
    private void buildSetRelationSection(UICommandBuilder cmd, UIEventBuilder events, Faction faction) {
        ...
        for (Faction other : factionManager.getAllFactions()) {
            if (!other.id().equals(factionId)) {
                ...
                if (relation == RelationType.NEUTRAL) neutralFactions.add(other);
            }
        }
    ...
    private String formatDate(long sinceMillis) {
        ...
        if (daysSince == 0) return "Since: today";
        else if (daysSince == 1) return "Since: 1 day ago";
        else return "Since: " + daysSince + " days ago";
    }
