 ...
 *
 * API methods used (from IRegionContainer):
 * - getRegionsAt(String, int, int, int) → Set<IRegion>
 * - canCreateClaim(String, BlockVector3, BlockVector3) → boolean
 * - getLoadedManagers() → Collection<IRegionManager>
 *
 * This is a soft dependency - if OrbisGuard is not installed, everything is a no-op.
 */
public final class OrbisGuardIntegration {
     ...
     *
     * Uses OrbisGuard's canCreateClaim API if available (checks all 8 corners + center
