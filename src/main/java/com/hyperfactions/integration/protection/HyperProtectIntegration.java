...
public final class HyperProtectIntegration {
    ...
    public static void registerAllHooks(@NotNull HyperFactions hf) {
        ...
        if (bridgeObj == null) {
            ...
            if (existing != null) bridgeObj = existing;
    ...
    public static void unregisterAllHooks() {
        Object bridgeObj = System.getProperties().get("hyperprotect.bridge");
        if (!(bridgeObj instanceof AtomicReferenceArray<?> rawBridge)) return;
    ...
    public static final class BlockBreakHook {
        ...
        BlockBreakHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class ExplosionHook {
        private final ProtectionChecker checker;
        ExplosionHook(ProtectionChecker checker) { this.checker = checker; }

        public int evaluateExplosion(Object world, int x, int y, int z) {
            try {
                String worldName = "";
                if (world != null) {
                    // Cross-classloader: World.getName() via reflection-safe approach
                ...
                Logger.debugMixin("Error in explosion hook: %s", e.getMessage());
    ...
    public static final class FireSpreadHook {
        private final ProtectionChecker checker;
        FireSpreadHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class BuilderToolsHook {
        private final ProtectionChecker checker;
        BuilderToolsHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class ItemPickupHook {
        private final ProtectionChecker checker;
        ItemPickupHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class DeathDropHook {
        private final ProtectionChecker checker;
        DeathDropHook(ProtectionChecker checker) { this.checker = checker; }
        ...
        public int evaluateDeathLoot(UUID playerUuid, String worldName, int x, int y, int z) {
            ...
            return keepInventory ? DENY_SILENT : ALLOW; // DENY = keep items (block the drop)
        }
    }
    ...
    public static final class DurabilityHook {
        private final ProtectionChecker checker;
        DurabilityHook(ProtectionChecker checker) { this.checker = checker; }
        ...
        public int evaluateWear(UUID playerUuid, String worldName, int x, int y, int z) {
            ...
            return prevent ? DENY_SILENT : ALLOW; // DENY = prevent durability loss
        }
    }
    ...
    public static final class ContainerAccessHook {
        private final ProtectionChecker checker;
        ContainerAccessHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class MobSpawnHook {
        private final ProtectionChecker checker;
        MobSpawnHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class TeleporterHook {
        private final ProtectionChecker checker;
        TeleporterHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class PortalHook {
        private final ProtectionChecker checker;
        PortalHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class EntityDamageHook {
        private final ProtectionChecker checker;
        EntityDamageHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class ContainerOpenHook {
        private final ProtectionChecker checker;
        ContainerOpenHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class BlockPlaceHook {
        private final ProtectionChecker checker;
        BlockPlaceHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class HammerHook {
        private final ProtectionChecker checker;
        HammerHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class UseHook {
        private final ProtectionChecker checker;
        UseHook(ProtectionChecker checker) { this.checker = checker; }

        public int evaluateUse(UUID playerUuid, String worldName, int x, int y, int z) {
            String reason = checker.checkUse(playerUuid, worldName, x, y, z);
            if (reason == null) return ALLOW;
            cachedReason.set(reason);
            return DENY_WITH_MESSAGE;
        }
    ...
    public static final class SeatHook {
        private final ProtectionChecker checker;
        SeatHook(ProtectionChecker checker) { this.checker = checker; }
    ...
    public static final class RespawnHook {
        private final HyperFactions hf;
        RespawnHook(HyperFactions hf) { this.hf = hf; }

        public double[] evaluateRespawn(UUID playerUuid, String worldName, int x, int y, int z) {
            return hf.getProtectionChecker().getRespawnOverride(playerUuid, worldName, x, y, z);
        }
    }
}
