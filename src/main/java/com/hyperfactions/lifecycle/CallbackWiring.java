...
public final class CallbackWiring {
  ...
  private static void wireEconomyCallbacks(HyperFactions hf) {
    ...
    EventBus.register(FactionDisbandEvent.class, event -> {
      ...
      java.math.BigDecimal balance = econ.getFactionBalance(faction.id());

      if (balance.compareTo(java.math.BigDecimal.ZERO) > 0 && ConfigManager.get().isEconomyDisbandRefundToLeader()) {
