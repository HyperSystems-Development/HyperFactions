...
public final class CommandUtil {
  ...
  /**
   * Parses raw arguments from a command input string by splitting on whitespace
   * and skipping the first N words (typically the command and subcommand).
   *
   * @param input the raw input string from {@code ctx.getInputString()}
   * @param skipWords number of leading words to skip (e.g., 2 for "/f create ...")
   * @return the remaining arguments, or empty array if none
   */
  @NotNull
  public static String[] parseRawArgs(@Nullable String input, int skipWords) {
    String[] parts = input != null ? input.trim().split("\\s+") : new String[0];
    return parts.length > skipWords ? Arrays.copyOfRange(parts, skipWords, parts.length) : new String[0];
  }
