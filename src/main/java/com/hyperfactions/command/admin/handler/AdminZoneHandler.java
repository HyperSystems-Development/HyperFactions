...
public class AdminZoneHandler {
    ...
    public void handleAdminZone(CommandContext ctx, @Nullable Store<EntityStore> store, @Nullable Ref<EntityStore> ref,
        ...
        switch (subCmd) {
            ...
            case "claim" -> {
                if (isPlayer) handleZoneClaim(ctx, worldName, chunkX, chunkZ, subArgs);
                else ctx.sendMessage(prefix().insert(msg("This command can only be used by a player.", COLOR_RED)));
            }
            case "unclaim" -> {
                if (isPlayer) handleZoneUnclaim(ctx, worldName, chunkX, chunkZ);
                else ctx.sendMessage(prefix().insert(msg("This command can only be used by a player.", COLOR_RED)));
            }
            case "radius" -> {
                if (isPlayer) handleZoneRadius(ctx, worldName, chunkX, chunkZ, subArgs);
                else ctx.sendMessage(prefix().insert(msg("This command can only be used by a player.", COLOR_RED)));
            }
